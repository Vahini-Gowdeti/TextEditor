...
const App: React.FC = () => {
  ...
  const [prevFont, setPrevFont] = useState<string>('');
  const [prevWeight, setPrevWeight] = useState<string>('');
  const [prevItalic, setPrevItalic] = useState<boolean>(false);
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      ...
      if (savedFont && fontData[savedFont]) {
        ...
        setItalicUnavailableMessage(fontData[savedFont].variants.some(v => v.includes('italic')) 
          ? '' 
        ...
        setItalicUnavailableMessage(fontData[firstFont].variants.some(v => v.includes('italic')) 
          ? '' 
          : 'Italic variants are not available for this font.');
      }
  ...
  useEffect(() => {
    ...
    const fontUrl = fonts[selectedFont]?.urls[0]; // Assuming the first URL is the correct one
    if (fontUrl) {
      loadFontIntoPage(selectedFont, fontUrl);
    }

    setItalicUnavailableMessage(fonts[selectedFont]?.variants.some(v => v.includes('italic')) 
      ? '' 
      : 'Italic variants are not available for this font.');
  ...
  const handleFontChange = (font: string) => {
    ...
    setItalicUnavailableMessage(fonts[font]?.variants.some(v => v.includes('italic')) 
      ? '' 
      : 'Italic variants are not available for this font.');
  };
  ...
  const handleSave = () => {
    ...
    setPrevFont(selectedFont);
    setPrevWeight(selectedWeight);
    setPrevItalic(isItalic);
  };
  ...
  const handleReset = () => {
    ...
    setItalicUnavailableMessage(fonts[defaultFont]?.variants.some(v => v.includes('italic')) 
      ? '' 
      : 'Italic variants are not available for this font.');
  ...
  return (
    <div className="App">
      ...
      <div className="previous-settings">
        <h2>Previously Saved Font</h2>
        <p><strong>Font:</strong> {prevFont || 'None'}</p>
        <p><strong>Weight:</strong> {prevWeight || 'None'}</p>
        <p><strong>Italic:</strong> {prevItalic ? 'Yes' : 'No'}</p>
      </div>
    </div>
  );
};
